...
def plotting(rad, den, virial_radius, virial_density,nfwfitp,burkertfitp,dehnen_threeparamfitp,dehnen_twoparamfitp,einastofitp):
    ...
    x, y = np.linspace(50000, 100000, 1000), np.linspace(0, 10, 1000)
    X, Y = np.meshgrid(x, y)
    ...
    #print(Z)
    pc = axs2.pcolor(Z,norm=matplotlib.colors.LogNorm(vmin=0.001, vmax=10000),cmap='PuBu_r', shading='auto')
    axs2.set_xlabel("Scale Density from 0 to 10 000")
    axs2.set_ylabel("Scale Radius from 0 to 1 000")
